...
public static void main(String[] args) {
	...
	rsColl.clear();
	RitHMSpecification rSpec1 =  new DefaultRiTHMSpecification("FORALL(ZOBJ)FORALL(YOBJ)FORALL(XOBJ)(<>a(XOBJ)->(! b(XOBJ) U a(XOBJ)))");
	rsColl.add(rSpec1);
	...
	for(int i= 0; i < Integer.parseInt(args[0]);i++){
		...
		for(int j = 0; j < Integer.parseInt(args[3]); j++)
		{
    		...
    		ps.setValue("ZOBJ", Integer.toString(rn.nextInt() % 100));
    		ps.setValue("YOBJ", Integer.toString(rn.nextInt() % 100));
