...
import rithm.parsertools.mtl.RitHMMTLVisitor;
import rithm.parsertools.ptltl.RitHMPTLTLVisitor;

public class PTLTLMonitor extends RitHMBaseMonitor implements RitHMMonitor{
	...
	/** The mtl mon. */
	protected RitHMPTLTLVisitor ptltlmon;
	...
	/** The Constant logger. */
	final static Logger logger = Logger.getLogger(PTLTLMonitor.class);
	
	public PTLTLMonitor(){
		currSpecStatus = new DefaultRiTHMSpecificationResult();
		bufferProgs = new DefaultProgStateCollection();
		ptltlmon = new RitHMPTLTLVisitor(bufferProgs);
	...
	public RitHMResultCollection runMonitor(boolean isLastInvocation) {
		...
		{
			String resName = ptltlmon.visit(specsTrees.get(currSpecs.at(i)));
			
			RitHMTruthValue tempTval = currSpecStatus.getResult(currSpecs.at(i));
			currSpecStatus.setResult(currSpecs.at(i), ptltlmon.getTruthValuation(resName,bufferProgs.length()-1));
		...
		for(int j = 0 ; j < currSpecs.length();j++)
		{
			...
			{
//				System.out.println("B: " + (long) (Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory()));
				String res = specToTreeNode.get(currSpecs.at(j));
				if(pipeMode){
					if(resPred.size() <= i)
						resPred.add(new DefaultPredicateState());

					if(Boolean.parseBoolean(ptltlmon.getTruthValuation(res, i).getTruthValueDescription()))
						resPred.get(i).setValue(specResmap.get(currSpecs.at(j)), 
								Boolean.parseBoolean(ptltlmon.getTruthValuation(res, i).getTruthValueDescription()));

					resPred.get(i).setTimestamp(ptltlmon.getTruthValuation(res, i).getTimetamp());
				}
				writeMonitoriLogFile("Event-No:<b>" + (i+1) +"</b><br>");
				writeMonitoriLogFile("timestamp: " + ptltlmon.getTruthValuation(res, i).getTimetamp() + "<br>");
				writeMonitoriLogFile("<div style=\"background: #B0B0B0 \">");
				if(ptltlmon.getTruthValuation(res, i).getTruthValueDescription().equals("true"))
					writeMonitoriLogFile(currSpecs.at(j).getTextDescription() + ":" + "<font color=\"Lime\">"+  ptltlmon.getTruthValuation(res, i).getTruthValueDescription() + "</font>");
				else
					writeMonitoriLogFile(currSpecs.at(j).getTextDescription() + ":" + "<font color=\"Red\">"+  ptltlmon.getTruthValuation(res, i).getTruthValueDescription() + "</font>");
				writeMonitoriLogFile("</div>");

				writeMonitorPlotFile(currSpecs.at(j).getTextDescription() + "," + ptltlmon.getTruthValuation(res, i).getTimetamp()+ "," + ptltlmon.getTruthValuation(res, i).getTruthValueDescription()+ "," + ptltlmon.getBufferSize() +"\n");
			}
		}
 		try {
 			if(isLastInvocation)
	...
	public void setPredicateEvaluator(PredicateEvaluator pe) {
		...
		ptltlmon.setPredicateEvaluator(pe);
	}
